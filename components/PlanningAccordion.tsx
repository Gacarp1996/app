// components/PlanningAccordion.tsx - VERSIÓN FINAL COMPLETA
...
const DEFAULT_CONFIG = {
 ...
 statusThreshold: THRESHOLDS.OPTIMAL,
 availableRanges: [
...
const PlanningAccordion: React.FC<PlanningAccordionProps> = ({ 
 player, 
 academiaId, 
 rangoAnalisis: rangoAnalisisProp,
 ...
 // Estado local solo si no viene del padre
 const [rangoAnalisisLocal, setRangoAnalisisLocal] = useState(
   rangoAnalisisProp || finalConfig.defaultRange
 );
 
 // Sincronizar con prop externa cuando cambie
 useEffect(() => {
   if (rangoAnalisisProp !== undefined) {
     setRangoAnalisisLocal(rangoAnalisisProp);
   }
 }, [rangoAnalisisProp]);
 
 // Handler para cambios de rango
 const handleRangoChange = (value: number) => {
   setRangoAnalisisLocal(value);
   onRangoAnalisisChange?.(value);
 };
 
 // Valor efectivo: usar prop si existe, sino local
 const rangoAnalisis = rangoAnalisisProp ?? rangoAnalisisLocal;
 
 // Usar el hook original que ya funcionaba
 const { 
   loading, 
   error, 
   analysisTree, 
 ...
 const getStatusConfig = (diferencia: number) => {
   ...
   const isDeficit = diferencia > 0;  // positivo = falta entrenar
 ...
 const renderNode = (node: AnalysisNode, level: number = 0, path: string = ''): React.ReactNode => {
   ...
   // ✅ CORREGIDO: Cálculo correcto del porcentaje de la barra
   const progressPercentage = node.planificado > 0 
     ? Math.min(150, (node.realizado / node.planificado) * 100)  // Permitir hasta 150% para mostrar excesos
     : node.realizado > 0 ? 100 : 0;  // Si no hay plan pero sí realizado, mostrar 100%
                     ...
                     <div className="w-full bg-gray-800 rounded-full h-2.5 overflow-hidden">
                       <div 
                         className={`h-full rounded-full transition-all duration-700 ease-out relative ${
                           // ✅ Usar el color correcto según el status
                           status.label === 'OK' ? 'bg-blue-500' :
                           status.label === 'Falta' ? 'bg-red-500' :
                           'bg-yellow-500'
                         }`}
     ...
     <div className="bg-gradient-to-r from-gray-900/80 to-gray-900/60 p-4 sm:p-6 rounded-xl border border-gray-800">
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
         ...
         <div className="flex items-center gap-3">
           ...
           {/* Selector de rango - Solo mostrar si NO viene del padre */}
           {!onRangoAnalisisChange && (
             <div className="flex items-center gap-2 bg-gray-800/50 px-3 py-2 rounded-lg">
               ...
               <select 
                 value={rangoAnalisis} 
                 onChange={(e) => handleRangoChange(Number(e.target.value))}

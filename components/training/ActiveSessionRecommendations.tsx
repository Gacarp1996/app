...
const ActiveSessionRecommendations: React.FC<ActiveSessionRecommendationsProps> = ({ 
  ...
  const analyzePlayerExercises = (playerId: string) => {
    ...
    // Mapear ejercicios directamente sin normalización
  ...
  const getIdealPercentageForAreaInType = (area: string, type: string, playerId: string) => {
    ...
    const idealPercentages: { [key: string]: { [key: string]: number } } = {
      [TrainingType.CANASTO]: {
        [TrainingArea.JUEGO_DE_BASE]: 17,
        [TrainingArea.JUEGO_DE_RED]: 17,
        [TrainingArea.PRIMERAS_PELOTAS]: 16
      },
      [TrainingType.PELOTEO]: {
        [TrainingArea.JUEGO_DE_BASE]: 15,
        [TrainingArea.JUEGO_DE_RED]: 10,
        [TrainingArea.PUNTOS]: 15,
        [TrainingArea.PRIMERAS_PELOTAS]: 10
      }
    };
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {[TrainingType.CANASTO, TrainingType.PELOTEO].map((tipo) => {
                ...
                {individualRecommendations ? (
                  <div className="space-y-4">
                    ...
                    {(() => {
                      ...
                      const mainTypes = [TrainingType.CANASTO, TrainingType.PELOTEO];
                                ...
                                {expandedRecommendations.has(`type-${tipo}` as any) && (
                                  <div className="p-4 bg-gray-900/30 border-t border-gray-600/30">
                                    ...
                                    {(() => {
                                      ...
                                      const allAreas = {
                                        [TrainingType.CANASTO]: [TrainingArea.JUEGO_DE_BASE, TrainingArea.JUEGO_DE_RED, TrainingArea.PRIMERAS_PELOTAS],
                                        [TrainingType.PELOTEO]: [TrainingArea.JUEGO_DE_BASE, TrainingArea.JUEGO_DE_RED, TrainingArea.PUNTOS, TrainingArea.PRIMERAS_PELOTAS]
                                      };
                       ...
                       {mainTypes.filter(tipo => {
                         ...
                         return (
                           <div key={tipo} className="bg-gray-800/50 border border-gray-600/50 rounded-xl overflow-hidden">
                             ...
                             {/* El resto del contenido expandible es idéntico al de la vista individual múltiple */}
                             {expandedRecommendations.has(`type-${tipo}` as any) && (
                               // Mismo contenido que en la vista individual múltiple
                               <div className="p-4 bg-gray-900/30 border-t border-gray-600/30">
                                 {/* ... contenido idéntico ... */}
                               </div>
                             )}

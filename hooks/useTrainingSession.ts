...
import { Player, Objective, TrainingSession, Tournament, LoggedExercise, SpecificExercise, TrainingType, TrainingArea } from '../types';
...
import { EXERCISE_HIERARCHY, getAvailableExercises, isValidTypeAreaCombination } from '../constants';
...
export const useTrainingSession = ({
  ...
  // Estados del formulario de ejercicio - ahora usando directamente los tipos
  const [currentTipo, setCurrentTipo] = useState<TrainingType | ''>('');
  const [currentArea, setCurrentArea] = useState<TrainingArea | ''>('');
  ...
  const { availableTipoKeys, availableAreaKeys, availableEjercicioNames } = useExerciseOptions(currentTipo, currentArea);
  ...
  const handleTipoChange = (value: string) => {
    setCurrentTipo(value as TrainingType);
    setCurrentArea('');
    setCurrentEjercicio('');
  ...
  const handleAreaChange = (value: string) => {
    setCurrentArea(value as TrainingArea);
    setCurrentEjercicio('');
    setCurrentEjercicioEspecifico('');
  };
      ...
      const playerExercises = exercises.filter(ex => ex.loggedForPlayerId === player.id)
        .map(({ loggedForPlayerId, loggedForPlayerName, ...rest }) => {
          ...
          const exercise: LoggedExercise = {
            id: rest.id || '',
            tipo: rest.tipo || TrainingType.PELOTEO,
            area: rest.area || TrainingArea.JUEGO_DE_BASE,
  ...
  return {
    ...
    // Estados del formulario - ahora con nombres más claros
    currentTipoKey: currentTipo,
    currentAreaKey: currentArea,
    currentEjercicioName: currentEjercicio,
    ...
    // Setters del formulario - usando los nombres actualizados
    setCurrentEjercicioName: setCurrentEjercicio,
    setCurrentEjercicioEspecifico,
    setTiempoCantidad,
    setIntensidad,
